...
export class AgentPactReportAccessError extends Error {
  constructor(message, status) {
    super(message);
    this.name = "AgentPactReportAccessError";
    this.status = status;
  }
}
...
export function agentPactReportMessage({ dealId, url, expires }) {
  const target = parseGitHubIssueUrl(url);
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(String(dealId ?? ""))) {
    throw new AgentPactReportAccessError("deal_id is invalid", 400);
  }
  const expiry = Number(expires);
  if (!Number.isSafeInteger(expiry)) {
    throw new AgentPactReportAccessError("expires is invalid", 400);
  }
  return {
    target,
    ...
    expires: expiry,
    message: [
      "ArgonautWorks AgentPact bounty report",
      `deal:${dealId}`,
      `url:${target.canonicalUrl}`,
      `expires:${expiry}`,
    ].join("\n"),
...
export async function authorizeAgentPactReport(query, expectedAddress, options = {}) {
  const now = options.now?.getTime?.() ?? Date.now();
  const normalized = agentPactReportMessage({
    dealId: query.deal_id,
    url: query.url,
    expires: query.expires,
  });
  if (normalized.expires < now) {
    throw new AgentPactReportAccessError("report link has expired", 410);
  }
  if (normalized.expires > now + MAX_LINK_LIFETIME_MS) {
    throw new AgentPactReportAccessError("report link lifetime is too long", 400);
  }
  if (!/^0x[0-9a-fA-F]{130}$/.test(String(query.signature ?? ""))) {
    throw new AgentPactReportAccessError("signature is invalid", 403);
  }
  const valid = await verifyMessage({
    address: expectedAddress,
    message: normalized.message,
  ...
  if (!valid) throw new AgentPactReportAccessError("signature is invalid", 403);
  return normalized;
}
...
function renderMarketplaceReport(result, transactionId, marketplace) {
...
<body><main>
  <p class="muted">ArgonautWorks · paid ${escapeHtml(marketplace)} ${escapeHtml(transactionId)}</p>
...
export function renderAtelierReport(result, orderId) {
  return renderMarketplaceReport(result, orderId, "Atelier order");
}

export function renderAgentPactReport(result, dealId) {
  return renderMarketplaceReport(result, dealId, "AgentPact deal");
}
