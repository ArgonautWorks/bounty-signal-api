...
export function classifyBountySignalTransfer(log, transaction, receivingWallet) {
  ...
  if (amount !== CHECK_PRICE_ATOMIC) return null;
...
export function revenueLedgerRow(receipt, date = new Date()) {
  const note = `Settled external x402 bounty check; Base transaction ${receipt.transaction}; payer ${receipt.payer}`;
  return [ledgerDate(date), "E014", "api_revenue", "0.00", "0.01", "0.01", note]
    .map(csvCell)
    .join(",");
}
